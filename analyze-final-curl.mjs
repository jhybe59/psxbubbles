...
async function runBacktest() {
    console.log("🔍 [Phase 1] Detecting Signals via Server-Side SQL (using curl.exe)...");
    ...
    try {
        ...
        console.error("Initial connection failed via curl:", e);
        return;
    }
    ...
    // Helper for date
    function n_date_iso(ts) {
        return ts.replace ? ts : new Date(ts).toISOString();
    }
    ...
    reportData.forEach(r => {
        // QuestDB timestamps often like '2026-01-08T09:58:12.000000Z'
