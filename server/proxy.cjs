/* Simple local proxy (CommonJS) to forward /api/* to https://psxterminal.com/api/*
   - Allows the browser to call http://localhost:3001/api/* without CORS problems
   - Usage: node server/proxy.cjs  (or npm run start-proxy)
*/
...
app.all('/api/*', async (req, res) => {
  try {
    ...
    if (typeof global.fetch !== 'function') {
      throw new Error('global fetch is not available in this Node runtime. Please use Node 18+ or run a different proxy.');
    }
    const upstream = await global.fetch(targetUrl, opts);
