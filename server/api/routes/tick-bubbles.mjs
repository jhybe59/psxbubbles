...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        for (const bubble of bubbles) {
            ...
            // orb_volume_5m is the volume from first 5 minutes
            const orbVol5m = bubble.orb_volume_5m || 0;
            const currentVol = bubble.volume || bubble.day_volume || 0;
            // For session RVOL, we compare "rate" - current volume per minute vs first 5 min rate
            // If we don't have exact minute data, use the rolling RVOL as fallback
