/**
 * Simple script to fetch all 75 symbols data at specific timestamp
...
const fetchKline = async (symbol, timestamp) => {
  ...
  try {
    ...
    const row = {
      ...
      ts: candleData.timestamp || timestamp,
    ...
    if (status === 503 || status === 429) {
      logger.warn({ symbol, timestamp, status }, 'Rate limit or service unavailable, will retry');
...
const fetchAllSymbols = async (symbols, timestamp) => {
  ...
  console.log(`\n📊 Fetching ${symbols.length} symbols with rate limit (600ms between requests)\n`);
    ...
    while (!success && retries < maxRetries) {
      try {
        ...
        if (row) {
          ...
          console.log(`⚠️  ${i + 1}/${symbols.length} ${symbol}: Not found or no data (404)`);
        ...
        if ((status === 503 || status === 429) && retries < maxRetries) {
          ...
          const waitMs = rateLimitMs * (retries + 1) * 2; // Exponential backoff
          console.log(`⚠️  ${i + 1}/${symbols.length} ${symbol}: Rate limit (${status}), waiting ${waitMs}ms before retry ${retries}/${maxRetries}`);
          await sleep(waitMs);
          continue;
        }
...
const fetchAndStore = async () => {
  try {
    ...
    const normalizedRows = allRows.map(row => ({
      ...
      daily_pct: row.dailyPct || row.intervalPct || null,
      raw: row.raw
    }));

    console.log(`4️⃣  Storing ${normalizedRows.length} rows in database...\n`);
